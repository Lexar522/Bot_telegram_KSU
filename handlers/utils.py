"""
Спільні утиліти для обробників
"""
import os
import json
import time
...
def _format_admission_2026(info: dict) -> str:
    ...
    parts.append(get_admissions_committee_phones())
    return "\n".join(parts)
...
def _check_and_fix_forbidden_universities(response: str, user_message: str) -> str:
    ...
    if has_forbidden:
        ...
        if any(word in user_message_lower for word in ["програмування", "програміст", "іт", "інформатика", "комп'ютер", "програмне", "121", "f6", "f2", "f3"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є такі спеціальності з програмування та інформаційних технологій: Інженерія програмного забезпечення (F2), Комп'ютерні науки (F3), Інформаційні системи та технології (F6, код 121).\n\n{get_admissions_committee_phones()} 💻"
        
        elif any(word in user_message_lower for word in ["медицина", "медичні", "лікар", "фармація", "реабілітація"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є такі спеціальності з охорони здоров'я: Фізична терапія, ерготерапія (бакалавр), Соціальна робота та консультування (бакалавр), Медицина (магістр), Фізична реабілітація (магістр), Фармація (магістр).\n\n{get_admissions_committee_phones()} 🏥"
        
        elif any(word in user_message_lower for word in ["освіта", "педагогіка", "вчитель", "дошкільна", "початкова"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є такі спеціальності з освіти та педагогіки: Дошкільна освіта, Початкова освіта, Спеціальна освіта (Логопедія, Олігофренопедагогіка), Середня освіта (різні спеціалізації), Фізична культура і спорт.\n\n{get_admissions_committee_phones()} 🎓"
        
        elif any(word in user_message_lower for word in ["право", "юрист", "юриспруденція"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є спеціальність Право (D8) на рівнях бакалавра та магістра.\n\n{get_admissions_committee_phones()} ⚖️"
        
        elif any(word in user_message_lower for word in ["економіка", "бізнес", "менеджмент", "фінанси"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є такі спеціальності з бізнесу та економіки: Економіка (С1.01), Менеджмент (D3), Фінанси, банківська справа та страхування (D2), Підприємництво та торгівля (D7).\n\n{get_admissions_committee_phones()} 💼"
        
        elif any(word in user_message_lower for word in ["психологія", "психолог"]):
            from knowledge_base import get_admissions_committee_phones
            return f"В ХДУ є спеціальність Психологія (С4) на рівнях бакалавра та магістра.\n\n{get_admissions_committee_phones()} 🧠"
            ...
            from knowledge_base import get_admissions_committee_phones
            return f"На жаль, не вдалося сформувати коректну відповідь. Звернися до приймальної комісії ХДУ для отримання актуальної інформації:\n\n{get_admissions_committee_phones()}"
